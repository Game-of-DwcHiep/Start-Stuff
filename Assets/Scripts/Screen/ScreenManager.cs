...
namespace DwcHyep
{
	public class ScreenManager : MonoSingleton<ScreenManager>
	{
		...
		private void Start()
		{
			...
			// neu la lan dau vao game thi cho choi luon level 1
			if(GameManager.Instance.LevelNumber == 1)
			{
				UIManager.Instance.OnMainPlayGame();
			}	
		...
		private void Update()
		{
			...
			{
				// Truoc tien hay thu va dong của so bat len dang hoat dong( neu co)
				if (!PopupManager.Instance.CloseActivePopup())
				{
					...
					{
                        string screenId = backStack[/*backStack.Count - 1*/ 0];
						...
						//UIController.Instance.OnActiveMenu();
                    }
				}
			}
		}
